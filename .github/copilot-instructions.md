...
- **TanStack Start serialization casts**: Server function return types must satisfy TanStack Start's serialization boundary. `unknown` is NOT serializable. When Drizzle `jsonb()` columns return `unknown`, we cast to `Record<string, {}> | null` (where `{}` means "any non-nullish value" in TypeScript). Do not suggest changing these casts to `Record<string, unknown>` -- it will cause compile errors.
